...
> RFPタブの「PDFをダウンロード」ボタンは、jsPDF＋日本語フォント埋め込みで**テキストベースのPDFを直接生成**します（画面の描画に依存しないため、スマホ/PCいずれでもA4で正しく出力されます）。日本語表示のため `public/fonts/ipag.ttf`（IPAゴシック）を同梱しています。jsPDF本体はCDN（jsdelivr/cdnjs）から実行時に読み込みます。
...
- 日本語PDF生成のため IPAゴシック（`public/fonts/ipag.ttf`）を同梱しています。IPAフォントライセンス（v1.0）に基づき再配布可能です。別フォントに差し替える場合は同パスのttfを置き換えてください。
